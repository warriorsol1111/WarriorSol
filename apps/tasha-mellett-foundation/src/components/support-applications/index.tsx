...
export default function AdminSupportApplicationsPage() {
  ...
  const handleAction = async (id: number, action: "accept" | "reject") => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/tasha-foundation/apply-for-support/${id}/${action}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <section className="w-full px-4 sm:px-6 md:px-8 lg:px-16 xl:px-24 py-10 min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto">
        {/* Page Heading */}
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-8 sm:mb-12 text-[#C1E965] text-left">
          Review Support Applications
        </h1>

        {loading ? (
          <div className="flex justify-center items-center py-24">
            <span className="animate-spin h-12 w-12 border-4 border-[#C1E965] border-t-transparent rounded-full" />
          </div>
        ) : error ? (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-base sm:text-lg font-bold text-[#C1E965]">
                ...
                <div className="mb-6 space-y-2 bg-gray-50 p-3 rounded-lg">
                  ...
                  {app.contactPhone && (
                    <div className="flex flex-wrap items-center text-sm">
                      ...
                      <span className="text-gray-600">{app.contactPhone}</span>
                    </div>
                  )}
                </div>
